...
```
postgresql://postgres:SENHA@db.railway.internal:5432/gyros_os
```
...
| ------------ | ------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------- |
| **API**      | `src/gyros_os/server/**`, `src/gyros_os/shared/**`, `pyproject.toml`, `uv.lock`, `Dockerfile.api`                              | Código da API + dependências compartilhadas    |
| **Worker**   | `src/gyros_os/worker/**`, `src/gyros_os/agents/**`, `src/gyros_os/shared/**`, `pyproject.toml`, `uv.lock`, `Dockerfile.worker` | Worker + agentes + dependências compartilhadas |
| **Frontend** | `frontend/**`                                                                                                                  | Isolado do backend                             |
| **DB**       | `db/**`, `Dockerfile.db`                                                                                                       | Migrações e imagem do Postgres                 |

### Por que não usar `src/**` para tudo?

O diretório `src/gyros_os/` contém código de ambos os serviços:
...
```python
# src/gyros_os/server/main.py (lifespan)
pool = await get_pool()
...
| `POSTGRES_DB`             | `gyros_os`                        | Nome do banco de dados                                    |
...
17. Testar login no painel e navegação completa
18. Testar fluxo completo: mensagem WhatsApp -> fila -> worker -> resposta
